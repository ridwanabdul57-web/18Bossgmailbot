...
def get_welcome_text(first_name):
    return (
        f"✨ *SELAMAT DATANG DI BOT SETORAN GMAIL V23* ✨\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if items_to_process:
        ...
        if inserted_count > 0:
            ...
            # Jika mode bulking, tampilkan password sekali di atas diikuti list email
...
def get_welcome_text(first_name):
    return (
        f"✨ *SELAMAT DATANG DI BOT SETORAN GMAIL V22* ✨\n"
        f"Halo *{first_name}*! Silakan baca informasi & aturan setoran di bawah ini:\n\n"
        f"💵 *INFORMASI RATE & PROSES*\n"
        f"• *Rate Per Akun:* Rp 4.000\n"
        f"• *Estimasi Pengecekan:* 24 - 48 Jam Kerja\n\n"
...
def get_welcome_text(first_name):
    return (
        f"✨ *SELAMAT DATANG DI BOT SETORAN GMAIL V20* ✨\n"
        f"Halo *{first_name}*! Silakan baca informasi & aturan setoran di bawah ini:\n\n"
        f"💵 *INFORMASI RATE & PROSES*\n"
        f"• *Rate Per Akun:* Rp 4.000\n"
        f"• *Estimasi Pengecekan:* 24 - 48 Jam Kerja\n\n"
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_utama":
        ...
        # Langkah 1: Pilih password dulu lewat tombol interaktif
        keyboard = [
            [InlineKeyboardButton("🔑 fineirga", callback_data="bulkpwd_fineirga")],
            [InlineKeyboardButton("🔑 sgsg1122", callback_data="bulkpwd_sgsg1122")],
            [InlineKeyboardButton("🔑 prabujaya", callback_data="bulkpwd_prabujaya")],
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif current_mode == 'BULKING_INPUT_EMAILS':
        chosen_password = context.user_data.get('bulk_password')
        gmail_pattern = r'^[a-zA-Z0-9._%+-]+@gmail\.com$'
        
        for line in lines:
            # Jika user mengirim format email:password secara tidak sengaja, ambil bagian depannya saja
            clean_line = line.split(':')[0].strip() if ':' in line else line
            if re.match(gmail_pattern, clean_line, re.IGNORECASE):
                items_to_process.append((clean_line.lower(), chosen_password))
...
if __name__ == '__main__':
    ...
    print("Bot Setoran V20 Aktif...")
    app.run_polling()
# ----------------- PESAN SAMBUTAN UTAMA -----------------
def get_welcome_text(first_name):
    return (
        f"✨ *SELAMAT DATANG DI BOT SETORAN GMAIL V18* ✨\n"
        f"Halo *{first_name}*! Silakan baca informasi & aturan setoran di bawah ini:\n\n"
        f"💵 *INFORMASI RATE & PROSES*\n"
        f"• *Rate Per Akun:* Rp 4.000\n"
        f"• *Estimasi Pengecekan:* 24 - 48 Jam Kerja\n\n"
        f"🔑 *ATURAN KATA SANDI (PASSWORD)*\n"
        f"• Password *WAJIB* menggunakan salah satu dari format berikut:\n"
        f"  1. `fineirga`\n"
        f"  2. `sgsg1122`\n"
        f"  3. `prabujaya`\n\n"
        f"⚠️ *SYARAT & KETENTUAN WAJIB*\n"
        f"1. *Dilarang Double-Sell:* Jangan pernah menjual kembali atau mengganti kata sandi akun selama proses verifikasi berlangsung.\n"
        f"2. *Email & Nomor Pemulihan:*\n"
        f"   • Email pemulihan *boleh terisi* / tidak masalah.\n"
        f"   • Nomor HP pemulihan *wajib dikosongkan* atau *jangan diverifikasi* (cukup ditautkan saja jika ada).\n"
        f"3. *Kondisi Akun (Good / Bad):*\n"
        f"   • Akun tergolong *Good* jika langsung menampilkan opsi memasukkan kata sandi saat pertama kali login, bukan tampilan verifikasi robot / captcha.\n"
        f"4. *Bebas Bug:* Akun tidak boleh terkena kunci akun, sesi bug, atau captcha berulang.\n\n"
        f"🚫 *SANKSI TEGAS:* Pelanggaran aturan di atas dapat mengakibatkan *PERMANENT BANNED* dari seluruh layanan setoran kami!\n\n"
        f"👇 *Pilih menu di bawah ini untuk memulai:* "
    )
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_utama":
        ...
        context.user_data['mode'] = 'BULKING'
        pesan = (
            "⏳ *MODE SETORAN BULKING AKTIF*\n"
            "═══════════════════════\n"
            "Silakan ketik dan kirimkan daftar Gmail kamu sekarang.\n\n"
            "📌 *Format:*\n"
            "`fineirga :` (atau sgsg1122 / prabujaya)\n"
            "`email1@gmail.com`\n"
            "`email2@gmail.com`\n"
            "`email3@gmail.com`\n\n"
            "_Sistem sedang menunggu inputan kamu..._"
    ...
    # ----------------- ADMIN ACC / REJECT WITHDRAW -----------------
        ...
        await query.edit_message_text(
            f"❌ *PENARIKAN #WD{wd_id} DITOLAK & SALDI DIREFUND!*\n\nNominal Rp {nominal:,} dikembalikan ke User `{target_uid}`.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("« Kembali ke Panel WD", callback_data="admin_withdraw")]]),
            parse_mode='Markdown'
        )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ----------------- PROSES REJECT PASTE LIST KHUSUS USER -----------------
    if current_mode == 'WAITING_USER_PASTE_REJECT':
        if user.id != ADMIN_CHAT_ID:
            return
    ...
    elif current_mode == 'BULKING':
        if len(lines) >= 2:
            first_line = lines[0]
            potential_password = first_line.split(':')[0].strip() if ':' in first_line else first_line
            gmail_pattern = r'^[a-zA-Z0-9._%+-]+@gmail\.com$'
            bulking_gmails = [line for line in lines[1:] if re.match(gmail_pattern, line, re.IGNORECASE)]
            
            if bulking_gmails and potential_password:
                for g in bulking_gmails:
                    items_to_process.append((g.lower(), potential_password))
    ...
    if items_to_process:
        ...
        if inserted_count > 0:
            msg_response += (
                ...
                f"📩 Total `{inserted_count}` akun Gmail telah diterima dan *menunggu persetujuan*.\n"
                f"⏳ Saldo tertahan: *Rp {inserted_count * HARGA_PER_GMAIL:,}*\n"
                f" (Akan otomatis berpindah ke Saldo Cair setelah disetujui admin).\n"
            )
        if duplicate_count > 0:
            msg_response += f"\n⚠️ `{duplicate_count}` akun ditolak otomatis karena sudah pernah dikirim sebelumnya."
        ...
        await update.message.reply_text(
            f"❌ *FORMAT SALAH!*\n\n"
            f"Kamu sedang dalam mode *SETOR {current_mode}*.\n"
            f"Mohon kirim ulang sesuai format, atau tekan tombol batal di bawah.",
            reply_markup=cancel_keyboard(),
            parse_mode='Markdown'
        )
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ----------------- ADMIN PANEL & KELOLA PENDING -----------------
        ...
        for wd_id, uid, uname, nom, met, rek, an, dt in wd_list:
            ...
            keyboard.append([
                InlineKeyboardButton(f"✅ Approve #{wd_id}", callback_data=f"accwd_{wd_id}"),
                InlineKeyboardButton(f"❌ Reject #{wd_id}", callback_data=f"rejwd_{wd_id}")
            ])
    ...
    # Tombol untuk memicu pilihan alasan sebelum paste list di user tertentu
    elif data.startswith("pasterej_ask_"):
        if user.id != ADMIN_CHAT_ID:
            await query.answer("❌ Akses khusus Admin!", show_alert=True)
            return
        ...
        # Ambil daftar email yang di-reject untuk notifikasi detail
        placeholders = ','.join(['?'] * len(selected_deps))
        cursor.execute(f'SELECT gmail FROM deposits WHERE id IN ({placeholders})', selected_deps)
        rejected_emails = [row[0] for row in cursor.fetchall()]
    ...
    # ----------------- ADMIN ACC / REJECT WITHDRAW -----------------
        ...
        await query.edit_message_text(
            f"❌ *PENARIKAN DITOLAK & SALDO DIKEMBALIKAN!*\n\nNominal Rp {nominal:,} dikembalikan ke User `{target_uid}`.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("« Kembali ke Panel WD", callback_data="admin_withdraw")]]),
            parse_mode='Markdown'
        )
        ...
        try:
            await context.bot.send_message(
                ...
                text=(
                    ...
                    f"═══════════════════════\n"
                    f"Permintaan penarikan kamu ditolak oleh admin.\n"
                    f"💰 *Saldo sebesar Rp {nominal:,} telah dikembalikan secara otomatis ke akun kamu.*"
                ),
                parse_mode='Markdown'
            )
        except Exception:
...
async def render_admin_user_deposits(query, target_uid, context):
    ...
    if not items:
        await query.edit_message_text("✅ *Tidak ada setoran pending untuk user ini.*", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("« Kembali ke Panel Admin", callback_data="admin_setoran")]]), parse_mode='Markdown')
        return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if current_mode == 'WAITING_USER_PASTE_REJECT':
        ...
        for email in emails_to_reject:
            # Pastikan hanya mereject email milik user yang sedang dibuka ini dan statusnya masih PENDING
            cursor.execute('SELECT id FROM deposits WHERE user_id = ? AND gmail = ? AND status = "PENDING"', (target_uid, email))
            row = cursor.fetchone()
        ...
        # Kirim notifikasi langsung ke user beserta daftar email dan alasannya
        if actually_rejected_emails:
            try:
                email_list_str = "\n".join([f"• `{e}`" for e in actually_rejected_emails])
                await context.bot.send_message(
    ...
    if current_mode == 'WAITING_REK':
        ...
        keyboard_admin = InlineKeyboardMarkup([
            [
                InlineKeyboardButton("✅ Approve WD", callback_data=f"accwd_{wd_id}"),
                InlineKeyboardButton("❌ Reject WD (Refund)", callback_data=f"rejwd_{wd_id}")
            ]
        ])
        ...
        await update.message.reply_text(
            ...
            f"📌 No Rek/HP: `{rekening}`\n"
            f"👤 Atas Nama: `{atas_nama}`\n\n"
            f"Permintaan penarikan kamu sedang diproses oleh admin.",
            reply_markup=main_menu_keyboard(user.id),
            parse_mode='Markdown'
        )
        return
